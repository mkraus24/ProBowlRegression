---
title: "Pro Bowl: Finding Snubs Using Regression"
...
x <- c("tidyverse", "data.table", "car", "nnet")
lapply(x, library, character.only = TRUE)
```

```{r code, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
...
probowl_multinom <- multinom(ProBowler ~ Pos + G + GS + Cmp + PassAtt + PassYds + PassTD + PassInt + RushAtt + RushYds + RushTD + Rec + RecYds + RecTD + DefInt + Sacks + Solo, data = players)
probowl_step <- step(probowl_multinom, trace = F, direction = 'both')
players[, probowl_prob := round(predict(probowl_step, players, 'probs'), 3)]
```
## Example
...
As you can see above, Aaron Rodgers was given a 99.7% chance to make the Pro Bowl based on the model. However, Tom Brady was selected with only a 20.5% chance to make it. 
...
Unfortunately, this model comes with a few notable drawbacks that one must not ignore.

1. The model does a very poor job of predicting Pro Bowl berths for defensive backs. Because interceptions are the only true statistic that the model has at its disposal, it essentially makes its decision of whether a cornerback or safety should be in the Pro Bowl based of that. Another variable like pass breakups, QBR allowed, or completion percentage when targeted would most likely be beneficial.

2. Conferences are not listed as variables. Excluding 2012-2015 (when players were drafted by NFL legends), the Pro Bowl teams are always separated by AFC and NFC. Just because the top six players in a certain position have the highest probability to be selected doesn't mean there will be room for them. Because the same number of players from the same position are chosen from each conference, some deserving players might be left out.

3. problem with probabilities